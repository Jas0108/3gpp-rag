...
Equipped with a **Multi-Query Hybrid Search Engine** (Dense + BM25 + Reciprocal Rank Fusion), **Cross-Encoder Reranking**, an **Evidence Sufficiency Gate**, an **Automatic LLM Failover System**, an **Interactive Terminal Chatbot**, and **Swagger UI Documentation**.
...
1. **Multi-Query Decomposition & Acronym-Aware Hybrid Search**:
   ...
   - **Acronym Expansion**: Automatically expands telecom acronyms (`MBSR`, `UCMF`, `5QI`, `S-NSSAI`, `PDU`).
...
2. **Cross-Encoder Reranking (`ms-marco-MiniLM-L-6-v2`)**:
   - Scores retrieved candidates with cross-attention and sigmoid normalization to filter out irrelevancies.

3. **Evidence Sufficiency Gate (Zero-Hallucination Guardrail)**:
   - Evaluates evidence quality *before* calling the LLM. If relevance score $< 0.01$, the engine abstains from answering rather than guessing.

4. **Automatic Model Failover System**:
   - Rotates seamlessly across top free models (`google/gemma-4-26b-a4b-it:free`, `nvidia/nemotron-3.5-lightning:free`, `liquid/lfm-2.5-2.6b:free`) to eliminate 429 rate limits or model availability errors.

5. **Section-Aware Ingestion Pipeline**:
   - Parses PyMuPDF pages while preserving 3GPP hierarchical section structures (`5.2.18.2`, `5.4.4.1`) and page numbers.

6. **Terminal Chatbot (`python run.py`) & Swagger UI**:
...
```
          ...
          (Zero Hallucination)              (Prompt | LLM Failover | Parser)
                                                       │
                                                       ▼
